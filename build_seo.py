...
<style>
  ...
  /* 320px: длинные русские слова в узкой колонке. Заголовок здесь на clamp
     с минимумом 34px, и на телефоне срабатывает именно минимум. hyphens переносит
     по слогам, break-word — страховка, если у браузера нет словаря языка.
     Обрезать заголовок нельзя ни при каких условиях. */
  @media (max-width: 400px) {{
    h1, h2, h3, .lead {{ -webkit-hyphens:auto; hyphens:auto; overflow-wrap:break-word; }}
  }}
...
SRV_CSS = '''
  ...
  h1 { font-size:clamp(34px,6vw,64px); line-height:1.05; margin:0 0 10px; letter-spacing:-.02em; }
  h2 { font-size:clamp(22px,3vw,32px); margin:52px 0 14px; }
  /* 320px: замер показал 14 обрезанных заголовков на /concepts/ и один на /atlas/
     после смены гарнитуры. Правило общее по тегам — следующий заголовок чинить
     не придётся. break-word рядом с hyphens не избыточен: без словаря языка
     hyphens молчит, и слово так и осталось бы обрезанным. */
  @media (max-width: 400px) {
    h1, h2, h3, .lead { -webkit-hyphens:auto; hyphens:auto; overflow-wrap:break-word; }
  }
...
OG_FILES = {
    ...
    'services/corporate-museum': 'og-corp-museum.jpg',
    'services/interactive-website': 'og-web.jpg',
    'services/showroom': 'og-space.jpg',
...
(function () {
  ...
  function apply() {
    ...
    try { dispatchEvent(new Event('pdfilter')); } catch (e) {}
  }
  function plural(n) {
