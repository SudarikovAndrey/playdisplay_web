...
EXCLUDES=(
  ...
  --exclude='/api/uploads/'
  # Сырьё компреда: исходники картинок, из которых вырезаны кадры страницы. Лежат внутри
  # site/, потому что владелец кладёт их туда с Мака, но на боевом сайте им делать нечего:
  # несколько мегабайт неиспользуемых файлов в папке, ссылку на которую отдают партнёру.
  --exclude='/avtovaz/material/'
  # 3D-гараж: модели (37 МБ) и Three.js. Слайд с живой сценой из компреда убран 17.08.2026,
  # страница на них больше не ссылается — но файлы оставлены на диске: сцена может
  # вернуться, а готовить модели заново дорого. На сервере им делать нечего.
  --exclude='/avtovaz/models/'
  --exclude='/avtovaz/vendor/'
  --exclude='/avtovaz/garage.html'
)
