...
EXCLUDES=(
  ...
  --filter='-s .DS_Store'
  # Локальные worktree Claude/Codex могут оказаться внутри site/. Это служебные файлы,
  # а не часть публичного сайта: не отправляем их и не удаляем одноимённое на сервере.
  --exclude='/.claude/'
  --exclude='/old/'
  ...
  --exclude='/layer/'
  # /immersive/ — «MR One», выложен на сервер мимо репозитория (обнаружен 30.08.2026
  # при сборке /web). Без этой строки rsync снёс бы живую страницу: в site/ её нет.
  # Появится в репозитории — строку снять.
  --exclude='/immersive/'
  # /ai-pack/dashboard/ — ещё один Next.js-экспорт, выложенный мимо репозитория.
  # Живой (отдаёт 200). Та же история, что у /layer/ и /immersive/.
  --exclude='/ai-pack/dashboard/'
