...
(function () {
  ...
  function football(x, y, z) {
    var p = SOCCER.p, b1 = -2, b2 = -2, j1 = 0, j2 = 0;
    for (var j = 0; j < p.length; j += 3) {
      var dot = x * p[j] + y * p[j + 1] + z * p[j + 2];
      if (dot > b1) { b2 = b1; j2 = j1; b1 = dot; j1 = j; }
      else if (dot > b2) { b2 = dot; j2 = j; }
    }
    /* Ширина шва — это РАССТОЯНИЕ ДО ГРАНИЦЫ, а не разница скалярных произведений.
     * Разница b1−b2 обращается в ноль на границе, но растёт от неё с разной скоростью:
     * у пары «пятиугольник — шестиугольник» центры сближены иначе, чем у пары
     * шестиугольников, а рядом с углом, где сходятся три панели, она мала на большой
     * площади. От этого шов пухнул на стыках в тёмные пятна, панели теряли углы, и
     * вместо раскройки получались неровные лоскуты. Граница двух ячеек — плоскость,
     * равноудалённая от центров; расстояние до неё = (p·a − p·b) / |a − b|, и вот оно
     * уже одинаково по всей длине шва. Порог 0.022 отдаёт шву 13 % площади (замер).
     */
    var ax = p[j1], ay = p[j1 + 1], az = p[j1 + 2];
    var bx = p[j2], by = p[j2 + 1], bz = p[j2 + 2];
    var len = Math.sqrt((ax - bx) * (ax - bx) + (ay - by) * (ay - by) + (az - bz) * (az - bz)) || 1;
    if ((b1 - b2) / len < 0.022 * WIDE) return 0;
    return SOCCER.k[j1 / 3] === 0 ? 1 : 2;   // пятиугольник тёмный, шестиугольник белый
  }
  ...
  function baseball(x, y, z) {
    nearCurve(SEAMPTS, x, y, z);
    if (_sd < 0.022 * WIDE) return 0;                // борозда шва — тонкая и тёмная
    // Стежки идут ДВУМЯ рядами вдоль шва, как на мяче: полоса взята не от нуля, а на
    // отступе от борозды, и расстояние до кривой симметрично — значит полоса сама
    // ложится с обеих сторон. Частые короткие штрихи, а не редкие пятна.
    var per = Math.round(6 * WIDE), on = Math.round(3 * WIDE);
    if (_sd > 0.028 * WIDE && _sd < 0.07 * WIDE && (_si % per) < on) return 2;
    return 1;                                        // белая кожа
  }
  ...
  function basket(x, y, z) {
    ...
    // шов — пропуск, поле — самое яркое. Шов узкий: на фотографии баскетбольного мяча
    // это тонкая чёрная линия, а не широкая полоса
    return _sd < 0.019 * WIDE ? 0 : (_sd < 0.033 * WIDE ? 1 : 2);
  }
  ...
  // Палитры сверены с фотографиями настоящих мячей: футбол чёрно-белый (тёмные
  // пятиугольники читаются провалом на тёмном фоне — как на снимке), баскетбол —
  // оранжевый с тонким чёрным швом, бейсбол — белая кожа с красными стежками.
  var BALLS = [
    // у футбола шов — самый тусклый (тонкая тёмная линия), пятиугольник — тёмно-серый,
    // но ВИДИМЫЙ: сделанный в ноль, он превращался в дыру, и белые шестиугольники
    // вокруг сливались в неопознаваемый лоскут
    { name: 'футбол',    fn: football, col: ['#070a12', '#39424f', '#f4f7fb'],
      al: [0.07, 0.46, 1.00], sz: [0.84, 0.95, 1.28] },
    { name: 'баскетбол', fn: basket,   col: ['#0d0805', '#8f4a18', '#ea8531'],
      al: [0.07, 0.42, 0.96], sz: [0.86, 0.95, 1.15] },
    ...
    { name: 'бейсбол',   fn: baseball, col: ['#0f1420', '#f2f6fc', '#e8321c'],
      al: [0.10, 0.90, 1.00], sz: [0.86, 1.00, 1.45] },
  ...
  /* Курсор-мяч живёт ТОЛЬКО на двух экранах: «Мяч — проводник» и финальный с контактами.
   * На остальных страницах книги возвращается прежний курсор — точка со свечением
   * (`.cursor-core`, её прячет класс `has-ball-cursor`). Причина простая: на страницах
   * с плотным текстом и фотографиями мяч в 15 % высоты экрана перекрывает содержание
   * и рвётся на каждой второй строке, а на этих двух он и есть содержание. */
  var ZONES = '.opening-statement, #contacts';
  function inZone(x, y) {
    var zs = document.querySelectorAll(ZONES);
    for (var i2 = 0; i2 < zs.length; i2++) {
      var b = zs[i2].getBoundingClientRect();
