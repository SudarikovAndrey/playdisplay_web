...
void main() {
  ...
  gl_FragColor = blend(bg,fg);
        ...
        setImages(imageFg, imageBg) {
            this.imageFg = imageFg;
            this.imageBg = imageBg;
            this.glObj.uniform1f("textureRatio", this.imageBg.width / this.imageBg.height);
            updateTexture(this.glObj.gl, 2, this.texFg, this.imageFg);
            updateTexture(this.glObj.gl, 3, this.texBg, this.imageBg);
        }
    }
    ...
    // ============================================================
    // 7) Boot (same renderer/drop logic, with the page photo as texture)
    // ============================================================
    function drawCover(source, target) {
        const ctx = target.getContext("2d");
        const sourceW = source.naturalWidth || source.videoWidth || source.width || 1;
        const sourceH = source.naturalHeight || source.videoHeight || source.height || 1;
        const scale = Math.max(target.width / sourceW, target.height / sourceH);
        const w = sourceW * scale;
        const h = sourceH * scale;
        const filter = source.dataset && source.dataset.rainFilter
            ? source.dataset.rainFilter
            : (source instanceof Element ? getComputedStyle(source).filter : "none");
    ...
    function makePhotoTextures(source, canvas) {
        const ratio = canvas.width / canvas.height;
        const bgW = Math.min(1280, Math.max(384, Math.round(canvas.width / Math.max(1, window.devicePixelRatio || 1))));
        const bgH = Math.max(256, Math.round(bgW / ratio));
        const fgW = Math.max(192, Math.round(bgW / 2));
        const fgH = Math.max(128, Math.round(bgH / 2));
        const fg = createCanvas(fgW, fgH);
    ...
    function startRaindrops() {
        ...
        loadImages([
            ...
            const source = window.RAINDROPS_SOURCE_IMAGE || document.getElementById("custom-bg") || tex;
            const drops = new Raindrops(canvas.width, canvas.height, dpr, dropAlpha, dropColor);

            drops.minR = 10;
            drops.maxR = 40;
            drops.rainChance = 0.35;
            drops.rainLimit = 6;
            drops.drizzle = 50;
            drops.drizzleSize = [3, 5.5];
            drops.trailRate = 1;
            drops.trailScaleRange = [0.2, 0.45];
            drops.globalTimeScale = Number(window.RAINDROPS_SPEED) || 1;

            const { fg, bg } = makePhotoTextures(source, canvas);
            ...
            window.__rainRenderer = renderer;
            window.__raindrops = drops;
            window.__rainSetSpeed = speed => {
                drops.globalTimeScale = Math.max(0, Number(speed) || 0);
            };
            window.__rainSetSource = sourceImage => {
                const textures = makePhotoTextures(sourceImage, canvas);
                renderer.setImages(textures.fg, textures.bg);
            };
        }).catch(err => {
            console.error(err);
        });
    ...
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", startRaindrops, { once: true });
    } else {
        startRaindrops();
    }
