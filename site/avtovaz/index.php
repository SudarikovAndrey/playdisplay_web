 ...
 * убран. Диагностика доступна как /avtovaz/?diag — она не раскрывает ни пароля, ни книги.
 */

$SALT = 'd9b9e885991c94cf62c7e5b031bd8b40';
$HASH = '371ecbac7fac3b51a600a75f7f806ef5b22eceeb7387e8f8e6b43e41add449b5';
$ITER = 200000;
$COOKIE = 'pd_avtovaz';
...
// ---- диагностика, если что-то не работает: /avtovaz/?diag ----
if (isset($_GET['diag'])) {
...
if (!$ok && $method === 'POST') {
    ...
    if ($pwd !== '' && pd_equals($HASH, pd_pbkdf2($pwd, $SALT, $ITER))) {
        ...
        header('Set-Cookie: ' . $COOKIE . '=' . $ticket . '; Expires=' . $exp
             . '; Path=/avtovaz/; HttpOnly; SameSite=Lax' . ($https ? '; Secure' : ''), false);
        $self = isset($_SERVER['REQUEST_URI']) ? strtok((string) $_SERVER['REQUEST_URI'], '?') : '/avtovaz/';
...
<title>Рынок Пацана × АвтоВАЗ · доступ по паролю</title>
