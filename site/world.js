...
const CONFIG = {
  ...
  density: {
    base: 0.44,            // шаг сетки у самого коридора; 0.53→0.47→0.44 (29.07):
                           // исправленное отсечение снимает ~37% невидимых точек —
                           // экономию возвращаем в плотность видимых поверхностей
...
function planLevel(seed, count) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // ПЕРВЫЕ ТРИ ЧАНКА — ВСЕГДА ОТКРЫТАЯ ПЕРЕДЫШКА (24.08). Раньше открытым был только
    // нулевой, а уже первый мог оказаться каньоном — на старте игрок ещё не взял руль
    // в руки, и «сразу столкновение, непонятно что случилось» шло в том числе отсюда.
    // Три TRANSITION подряд ≈ 500-700 юнитов ≈ 5-7 секунд спокойного полёта над
    // простым рельефом, и только потом словарь приёмов начинает вводиться по одному.
    if (i < 3) type = T.TRANSITION;
