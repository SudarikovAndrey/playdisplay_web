...
const CONFIG = {
  ...
  density: {
    base: 0.62,            // шаг сетки у самого коридора (юнитов между точками)
    ...
    budget: 300000         // §23 бюджет активных точек
  },
...
function makeChunkDef(o) {
  ...
  switch (o.type) {
    case T.TRANSITION:
      ...
      def.params = { wallH: lerp(10, 20, d), landmark: r() < 0.7 };
      break;
    case T.CANYON:
      def.corridorW = lerp(46, 30, d); def.sway = 22; def.curve = 1.6;
      def.vertical = 0.7; def.yBase = 17;
      def.params = { wallH: lerp(34, 62, d), depth: lerp(26, 48, d),
    ...
    case T.GORGE:
      ...
      def.params = { wallH: lerp(48, 74, d), ledges: 2 + Math.floor(r() * 3),
