...
function pd_config() {
  ...
  $defaults = array(
    // Куда уходит бриф ассистента. Туда же идут и заявки формы брони, но у них
    // адрес продублирован в коде — см. pd_action_book() в ai.php.
    //
    // ЭТО ТОЛЬКО ЗНАЧЕНИЕ ПО УМОЛЧАНИЮ, и на боевом оно НЕ ДЕЙСТВУЕТ. Там лежит
    // ~/wordpress_2/pd-ai-config.php со своей строкой mail_to, и она побеждает;
    // деплой этот файл не трогает (он вне репозитория и вне rsync). Именно так
    // 23.08.2026 бриф ушёл на прежний адрес, хотя здесь уже был правильный:
    // правку сделали в коде, а решает сервер. МЕНЯТЬ АДРЕС НАДО В ДВУХ МЕСТАХ —
    // здесь для порядка и в конфиге на сервере, иначе смысла нет.
