...
// ---------- сложность ----------
/**
 * Сколько нулевых бит требовать. Каждый бит удваивает работу, поэтому потолок здесь
 * низкий и подобран замером: наша реализация sha256 в браузере даёт около 800 тысяч
 * хешей в секунду, телефон — вчетверо меньше. 16 бит это 0,1 с на компьютере и 0,3 с
 * на слабом телефоне, 19 бит — 0,6 и 2,6 с. Выше поднимать нельзя: за 22 бита телефон
 * считал бы двадцать секунд, а живой человек за один вечер вполне может открыть панель
 * десять раз — и он не должен за это расплачиваться.
 */
function pd_guard_bits() {
  ...
  $max = isset($cfg['guard_bits_max']) ? (int)$cfg['guard_bits_max'] : 19;
