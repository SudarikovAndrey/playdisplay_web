...
(function () {
  ...
  /* АВТОМАТИЧЕСКИЕ БРАУЗЕРЫ НЕ СЧИТАЕМ. Отметка ?pdoff=1 ниже требует, чтобы
     ...
     navigator.webdriver выставляют в true все инструменты автоматизации —
     панель браузера в Claude, Playwright, Puppeteer, Selenium. Это часть
     стандарта HTML, а не хитрость: браузер обязан так себя пометить, когда
     им управляет программа. Живому посетителю выставить его нечем, значит
     ложных отказов не будет.

     Проверка стоит ДО отметки ?pdoff=1 и не отменяет её: отметка нужна
     для обычного браузера, в котором сайт открывает человек из студии. */
  if (navigator.webdriver === true) NOTRACK = true;
