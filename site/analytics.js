...
(function () {
  ...
  /* АВТОМАТИЧЕСКИЕ БРАУЗЕРЫ НЕ СЧИТАЕМ. Отметка ?pdoff=1 ниже требует, чтобы
     человек про неё помнил и поставил в каждом браузере. На деле не помнит:
     за 25.08–24.09.2026 на /pt/ пришлось 144 просмотра от ОДНОГО посетителя —
     это сборка португальской версии, восемнадцатого числа видно 6 визитов
     и 159 просмотров с глубиной 26,5. Восемнадцать процентов месячной
     статистики оказались отладкой.

     Двух проверок мало по отдельности, поэтому стоят обе.

     navigator.webdriver — часть стандарта HTML: браузер обязан так себя
     пометить, когда им управляет программа. Ловит Playwright, Puppeteer,
     Selenium, headless-прогоны.

     НО проверено 24.09.2026 на живом сайте: встроенная панель браузера
     в Claude отдаёт navigator.webdriver === false и счётчик в ней
     запускается. Одного этого признака НЕ ХВАТАЕТ, и именно такой браузер
     вероятнее всего и накликал те 144 просмотра. Зато он честно называет
     себя в User-Agent строкой Claude/<версия> — по ней и отсекаем.

     Список UA умышленно короткий и из продуктовых токенов, которые в живом
     браузере не появляются. Подстроку 'bot' сюда не добавляем: она встречается
     в безобидных строках и отрезала бы настоящих людей.

     Проверки стоят ДО отметки ?pdoff=1 и не отменяют её: отметка нужна для
     обычного браузера, в котором сайт открывает человек из студии, — например
     для расширения Claude в Chrome, которое тоже не ставит webdriver и своего
     токена в UA не имеет, потому что это настоящий Chrome пользователя. */
  var AUTO = /HeadlessChrome|Claude\/|Electron\/|PhantomJS|Playwright|Puppeteer|Lighthouse/i;
  if (navigator.webdriver === true || AUTO.test(navigator.userAgent)) NOTRACK = true;
