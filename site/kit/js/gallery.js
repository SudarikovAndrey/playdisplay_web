  ...
  function setup(root) {
    ...
    /* ── ВЕРТИКАЛЬНАЯ ПРОКРУТКА ЛИСТАЕТ КАДРЫ, ПОКА ОНИ НЕ КОНЧИЛИСЬ ──────
       ...
       Три условия, без которых перехват вредит:
        ...
        • ОДИН ЖЕСТ — ОДИН КАДР, и держится это ДВУМЯ условиями сразу. Только пауза по
          времени (пробовали 520 мс) не спасает: инерция тачпада на Маке присылает события
          больше секунды, и один свайп проходил два-три кадра. Только «жест закончился
          после тишины» тоже не спасает: медленная прокрутка колесом даёт события с
          промежутками БОЛЬШЕ порога тишины, каждое считается новым жестом, и лента опять
          улетает через несколько кадров. Поэтому здесь и то и другое:
            – пока идёт тот же жест (события чаще, чем раз в 160 мс), шаг только один;
            – между шагами железный пол 480 мс — чуть меньше, чем переход трека (500 мс),
              чтобы следующий кадр начинал ехать сразу, как только доехал прежний.
          Так «заедает» и «мотает через три кадра» лечатся одним правилом: ход ленты
          привязан к её собственной скорости, а не к тому, как часто пришло колесо.
    ...
    host.addEventListener('wheel', function (e) {
      ...
      e.preventDefault();
      /* ОДНО ПРАВИЛО: НЕ ЧАЩЕ, ЧЕМ ЕДЕТ САМА ЛЕНТА. Между шагами пол 480 мс — чуть меньше
         перехода трека (500 мс), поэтому следующий кадр начинает ехать сразу, как доехал
         прежний, но обогнать себя лента не может.
         Так вышло не сразу, и обе прежние попытки владелец забраковал по делу:
          • только пауза 520 мс — инерция тачпада присылает события больше секунды, и один
            свайп проходил два-три кадра («мотает через несколько слайдов»);
          • «шаг на первом событии жеста, жест кончается после тишины» — во время долгой
            инерции все события съедались молча, и лента стояла целую секунду («заедает»),
            а медленная прокрутка колесом с промежутками больше порога тишины опять давала
            залп шагов.
         Пол лишён обеих болезней: лента отвечает всегда и никогда не быстрее анимации.
         ОТМЕТКА ЖИВЁТ НА САМОМ ЭЛЕМЕНТЕ, а не в замыкании: если setup() для галереи
         выполнится дважды, обработчиков станет два, и у каждого был бы свой счётчик —
         пол перестал бы держать. */
      var now = Date.now();
      if (now - Number(root.dataset.wheelStep || 0) < 480) return;
      root.dataset.wheelStep = String(now);
