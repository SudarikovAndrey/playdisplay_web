...
(function () {
  ...
  function setup(root) {
    ...
    /* ── ВЕРТИКАЛЬНАЯ ПРОКРУТКА ЛИСТАЕТ КАДРЫ, ПОКА ОНИ НЕ КОНЧИЛИСЬ ──────
       ...
       по горизонтали. */
    var gesture = false, gestureEnd = null, lastStep = 0;
    ...
    host.addEventListener('wheel', function (e) {
      ...
      e.preventDefault();
      /* Хвост инерции продолжаем гасить: пока события идут, жест тот же самый */
      clearTimeout(gestureEnd);
      gestureEnd = setTimeout(function () { gesture = false; }, 160);
      if (gesture) return;
      var now = Date.now();
      if (now - lastStep < 480) return;      // лента ещё едет — второй шаг был бы рывком
      gesture = true;
      lastStep = now;
