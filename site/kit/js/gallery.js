  ...
  function setup(root) {
    ...
    /* ── ВЕРТИКАЛЬНАЯ ПРОКРУТКА ЛИСТАЕТ КАДРЫ, ПОКА ОНИ НЕ КОНЧИЛИСЬ ──────
       ...
       Три условия, без которых перехват вредит:
        ...
        • ОДИН ЖЕСТ — ОДИН КАДР, и считать надо именно жест, а не время. Первая версия
          держала паузу 520 мс, и её не хватило: у тачпада на Маке инерция присылает
          события больше секунды, то есть один физический свайп проходил два-три кадра.
          Теперь шаг делается на ПЕРВОМ событии жеста, а жест считается законченным после
          140 мс тишины — сколько бы событий инерция ни прислала, кадр сменится один раз.
       ...
       по горизонтали. */
    var gesture = false, gestureEnd = null;
    ...
    host.addEventListener('wheel', function (e) {
      ...
      gestureEnd = setTimeout(function () { gesture = false; }, 140);
      if (gesture) return;
      gesture = true;
