...
(function () {
  ...
  function draw() {
    ...
    /* КАМЕРА ОПУСКАЕТСЯ, А НЕ ПОДНИМАЕТСЯ — в этом вся суть движения.
     *
     * В начале камера строго над площадкой и смотрит вниз: поле лежит параллельно
     * экрану, как план. По мере прокрутки камера СНИЖАЕТСЯ к плоскости и доворачивается
     * к горизонту; на уровне земли плоскость проецируется в одну линию — поле само
     * сжимается в черту и гаснет. Первая попытка делала обратное (камера уходила вверх,
     * масштаб падал), и это читалось как «поле улетает», а не как опускание камеры.
     *
     * Вместе со снижением камера ОТЪЕЗЖАЕТ НАЗАД (`back`). Без этого ближняя половина
     * площадки оказывается позади камеры и просто исчезает, вместо того чтобы уйти
     * в перспективу.
     */
    /* КАМЕРА СТОИТ ЗА НИЖНЕЙ ГРАНЬЮ. Это и определяет, что на нас наезжает.
     * Расстояние до точки считается как `back − v`: у нижней грани (v = +1) оно самое
     * маленькое, значит она ближе всех — она и наплывает на камеру, разъезжаясь вниз
     * и в стороны. Верхняя грань (v = −1) самая дальняя: она сжимается и уходит к
     * горизонту. Прежняя версия складывала `v + back`, то есть камера стояла за ВЕРХНЕЙ
     * гранью, и на зрителя наезжал дальний край — ровно наоборот.
     *
     * `back` не опускается ниже единицы: иначе при наклоне нижний край площадки
     * оказывается позади камеры и просто пропадает посреди движения.
     *
     * Знак экранной оси: у ближней точки числитель (fwd·cos a − cam·sin a) отрицателен,
     * у дальней положителен, поэтому берём его с минусом — иначе перспектива
     * переворачивается и близкое уезжает вверх.
     */
    var ang = p * TILT;
    var ca = Math.cos(ang), sa = Math.sin(ang);
    var cam = CAM * (1 - p) + 0.05 * p;            // высота: от «сверху» до уровня земли
    // Камера ПРИБЛИЖАЕТСЯ: расстояние до площадки убывает, а не растёт. С растущим
    // расстоянием картинка мельчала, и это читалось как «отлетаем от разметки».
    // Ниже единицы не опускаемся — иначе ближняя грань окажется позади камеры.
    var back = 2.4 - 1.25 * p;
    var F = Math.min(W, H) * 0.46;
    var cx = W * 0.5 + (p - 0.5) * W * 0.05;       // лёгкий сдвиг — тот самый параллакс
    var cy = H * 0.5;                              // смотрим ровно в центр поля
    var P = live.court;
    // центр площадки держим на месте: иначе при наклоне вся картинка уползает по экрану
    var y0 = -(back * ca - cam * sa) / (cam * ca + back * sa);

    for (i = 0; i < P.length; i += 2) {
      var u = P[i], fwd = back - P[i + 1];
      if (fwd < 0.03) continue;                    // позади камеры
      var d = cam * ca + fwd * sa;
      if (d < 0.05) continue;
      var sx = cx + (u / d) * F;
      var sy = cy + (-(fwd * ca - cam * sa) / d - y0) * F;
