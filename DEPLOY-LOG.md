...
## 2026-09-24 17:56 — hybrid-v8 @ f89e88a

**Коммиты в этой поставке**
```
f89e88a Логи хостинга включены: переключатель врал
e1def97 Журнал деплоя: e5a5fa7
```

**На сервер отправлено файлов:** 109, удалено: 0
```
.htaccess
llms.txt
robots.txt
...
## 2026-09-24 17:11 — hybrid-v8 @ e5a5fa7

**Коммиты в этой поставке**
```
e5a5fa7 Отсечение автоматических браузеров: одного webdriver не хватило
6971ac6 Журнал деплоя: e9fa92e
```

**На сервер отправлено файлов:** 106, удалено: 0
```
analytics.js
index.html
llms.txt
...
## 2026-09-24 17:09 — hybrid-v8 @ e9fa92e

**Коммиты в этой поставке**
```
e9fa92e Журнал деплоя: 4c6121d
```

**На сервер отправлено файлов:** 104, удалено: 0
```
llms.txt
robots.txt
sitemap.xml
...
## 2026-09-24 17:08 — hybrid-v8 @ 4c6121d

**Коммиты в этой поставке**
```
4c6121d Аналитика: не считать автоматические браузеры
0258d88 Журнал деплоя: aa45ffa
```
...
**Коммиты в этой поставке**
```
c56b5b2 Генератор: длинные слова на 320px больше не обрезаются
313068c Журнал деплоя: eab7d1a
```
...
## 2026-09-24 16:05 — hybrid-v8 @ eab7d1a

**Коммиты в этой поставке**
```
eab7d1a Выделение текста выключено; узкий экран: два заголовка не влезали
e2f2faf Журнал деплоя: 7678f2c
```
...
## 2026-09-24 15:52 — hybrid-v8 @ 7678f2c

**Коммиты в этой поставке**
```
7678f2c Вес заголовков +150: компенсация смены гарнитуры
6802673 Журнал деплоя: 7a01099
```

**На сервер отправлено файлов:** 107, удалено: 0
```
index.html
landing.css
llms.txt
...
**Коммиты в этой поставке**
```
1ec40f7 «Артек» вышел с закупкой на 156,5 млн, добавлен Эрмитаж
1c72fb6 Старые адреса из выдачи вели в 404, бэкапы /layer/ индексировались
ca0a47a Журнал деплоя: 69e2f2b
```

**На сервер отправлено файлов:** 73, удалено: 0
```
.htaccess
404.html
llms.txt
...
**Коммиты в этой поставке**
```
4c7d0f4 Цифровой слой: доступ только по ссылке, из поиска убрана
fa35631 Журнал деплоя: e657a87
```
...
## 2026-09-18 11:31 — hybrid-v8 @ e657a87

**Коммиты в этой поставке**
```
e657a87 Светлая тема: номера разделов синим
cd2697e Светлая тема: номера разделов синим вместо кораллового
ff19fde Журнал деплоя: 53b62ac
```

**На сервер отправлено файлов:** 72, удалено: 0
```
llms.txt
robots.txt
sitemap.xml
...
**Коммиты в этой поставке**
```
d663316 MARÉ: телефону вдвое меньше пикселей, интерфейс без размытия фона (снимок ocean-demo)
9a5f58a Журнал деплоя: 20a595e
```

**На сервер отправлено файлов:** 92, удалено: 9
```
llms.txt
robots.txt
sitemap.xml
...
## 2026-09-08 19:51 — hybrid-v8 @ 20a595e

**Коммиты в этой поставке**
```
20a595e MARÉ: инерция камеры втрое короче — доворот 10° вместо 30 (снимок ocean-demo)
5e0382a Журнал деплоя: d4e36f0
```
...
## 2026-09-08 19:35 — hybrid-v8 @ d4e36f0

**Коммиты в этой поставке**
```
d4e36f0 MARÉ: свайп камеры без рывков, флик доворачивает и затухает (снимок ocean-demo)
94ef9d1 Найдена чужая карточка, которая портит позиционирование
96de2d9 Журнал деплоя: 60770a5
```
...
**Коммиты в этой поставке**
```
1ad8f9b Прототипы /mare/ и /poemiq/ закрыты от индексации
8ccc28c Журнал деплоя: d593e48
```
...
**Коммиты в этой поставке**
```
a551390 POEMIQ: камни без рывков (per-object данные в шейдере), общие настройки с паролем владельца (settings.php), цепь без тени, музыка только при активной вкладке
2e22cbe Журнал деплоя: eb5804d
```
...
**Коммиты в этой поставке**
```
a52c45c Стили хаба концепций вынесены из общей базы
278936e Концепции — одной страницей: 56 адресов схлопнуты в хаб с якорями
4f1ecc0 Журнал деплоя: 817accd
```

**На сервер отправлено файлов:** 73, удалено: 112
```
.htaccess
index.html
llms.txt
...
## 2026-09-06 22:25 — hybrid-v8 @ 817accd

**Коммиты в этой поставке**
```
817accd Лёгкие копии видео — не только телефону, но и медленному каналу
bcb9224 MARÉ: кольцо в небе рисуется карточками, телефону не нужно трассировать камень
058d93d Журнал деплоя: 22a577f
```

**На сервер отправлено файлов:** 147, удалено: 8
```
hero-scene.html
index.html
llms.txt
...
## 2026-09-06 21:25 — hybrid-v8 @ 22a577f

**Коммиты в этой поставке**
```
22a577f POEMIQ: мобильная вёрстка, логотип без излома, GPU-ирис заставки, плавная шапка, листалка во всю ширину
1aec410 Журнал деплоя: 9e8ff0d
```

**На сервер отправлено файлов:** 127, удалено: 0
```
llms.txt
robots.txt
sitemap.xml
...
## 2026-09-06 16:33 — hybrid-v8 @ 9e8ff0d

**Коммиты в этой поставке**
```
9e8ff0d POEMIQ: люкс-фотосерия с листалкой, SSR-отражения, заставка с логотипом, новые модели колец и цепи
8450412 Журнал деплоя: 7ff60bf
```
...
**Коммиты в этой поставке**
```
2434dd6 POEMIQ опубликован на playdisplay.com/poemiq и заведён на витрине
aa5d4cd Журнал деплоя: 2641c3b
```

**На сервер отправлено файлов:** 130, удалено: 0
```
llms.txt
robots.txt
sitemap.xml
...
## 2026-09-05 23:44 — hybrid-v8 @ 2641c3b

**Коммиты в этой поставке**
```
2641c3b MARÉ обновлён: новая вода, модель, HDR-окружения и звук
3214d7f Журнал деплоя: bce1657
```

**На сервер отправлено файлов:** 148, удалено: 10
```
llms.txt
robots.txt
sitemap.xml
...
## 2026-09-05 23:26 — hybrid-v8 @ bce1657

**Коммиты в этой поставке**
```
bce1657 Обложка WORLD — кадр владельца, ночной мир №444815
302824c Журнал деплоя: 0ab1c50
```

**На сервер отправлено файлов:** 128, удалено: 0
```
llms.txt
robots.txt
sitemap.xml
...
## 2026-09-05 23:18 — hybrid-v8 @ 0ab1c50

**Коммиты в этой поставке**
```
0ab1c50 Витрина /web в оформлении библиотеки, карточки крупнее
850d97b Витрина /web оформлена как «Библиотека решений», карточки крупнее
b314f30 Журнал деплоя: 0b76a74
```
...
## 2026-09-05 19:57 — hybrid-v8 @ 0b76a74

**Коммиты в этой поставке**
```
0b76a74 Витрина: переснят кадр sudarikov.space/world
98c1744 Журнал деплоя: 3fd9bde
```
...
## 2026-09-05 18:34 — hybrid-v8 @ 3fd9bde

**Коммиты в этой поставке**
```
3fd9bde MARÉ опубликован на playdisplay.com/mare статическим снимком
92acdc9 Журнал деплоя: 75990e2
```

**На сервер отправлено файлов:** 146, удалено: 0
```
llms.txt
robots.txt
sitemap.xml
...
**Коммиты в этой поставке**
```
1daf1a3 Витрина веб-прототипов: playdisplay.com/web
d06e505 Витрина веб-прототипов: playdisplay.com/web
c302a2f Пять писем первого касания с контактами из ЕИС
f5382cd Журнал деплоя: dafc983
```
...
ai-pack/index.html
ai-pack/universal.html
ai-pack/assets/ai-pack-hero.jpg
ai-pack/assets/ai-pack-omnichannel.jpg
ai-pack/assets/ai-pack-pos.jpg
akvion/index.html
atlas/index.html
avtovaz/private/page.html
concepts/index.html
...
## 2026-08-31 22:28 — hybrid-v8 @ dafc983

**Коммиты в этой поставке**
```
dafc983 Пыль на сетке проектов больше не залипает: pdActive не верит одному hasFocus()
137ada0 Кейсы для Workspace: тексты и поправка на таксономию площадки
e6f2220 Журнал деплоя: ccfcc77
```
...
## 2026-08-31 15:50 — hybrid-v8 @ ccfcc77

**Коммиты в этой поставке**
```
ccfcc77 Курсор больше не двоится: арт-«плюс» виден только там, где скрыт системный
26214f4 Журнал деплоя: e4ea439
```
...
## 2026-08-31 15:27 — hybrid-v8 @ e4ea439

**Коммиты в этой поставке**
```
e4ea439 boxingcartel: фикс кнопки назад из карточки
842fe5c Журнал деплоя: 7da094e
```
...
## 2026-08-31 15:04 — hybrid-v8 @ 7da094e

**Коммиты в этой поставке**
```
7da094e boxingcartel: единая гамма hero
8ca78aa Журнал деплоя: ac9590a
```

**На сервер отправлено файлов:** 133, удалено: 0
```
llms.txt
robots.txt
sitemap.xml
...
**Коммиты в этой поставке**
```
09cd029 «Попробуем сейчас» поднят пятым слайдом
f6e1284 Журнал деплоя: 5a5ac9d
```
...
## 2026-08-30 11:36 — hybrid-v8 @ 5a5ac9d

**Коммиты в этой поставке**
```
5a5ac9d Insight Engine парами «инсайт → действие»; цена пилота убрана; портрет аудитории влезает по высоте
3f48269 Журнал деплоя: 26ded27
```
...
## 2026-08-30 11:23 — hybrid-v8 @ 26ded27

**Коммиты в этой поставке**
```
26ded27 Пятый источник ценности – персонал и продавцы
a47090b Журнал деплоя: d3f06ed
```
...
## 2026-08-30 11:20 — hybrid-v8 @ d3f06ed

**Коммиты в этой поставке**
```
d3f06ed Слайд «Разные слои информации»: персонал, врачи и фармацевты, онбординг
c038f57 Журнал деплоя: 28e41a3
```
...
**Коммиты в этой поставке**
```
b58d94f Аудит доступа: закрыты архивы и черновики
ca4f9a1 Аудит открытого доступа: закрыты архивы и черновики
b2bf82a Журнал деплоя: 710728d
```
...
## 2026-08-27 13:15 — hybrid-v8 @ 710728d

**Коммиты в этой поставке**
```
710728d Тело астероида — шейдер-камуфляж неба; ядра цепочек раздвинуты
c6e7745 Журнал деплоя: 655faa2
```
...
**Коммиты в этой поставке**
```
3f7846a Астероиды точками в стиле сцены; развилки чаще, диагональные шпили, давление потолка
8fe122f Журнал деплоя: cffefd4
```
...
## 2026-08-26 23:46 — hybrid-v8 @ cffefd4

**Коммиты в этой поставке**
```
cffefd4 Астероиды перерисованы: сплошные огранённые тела вместо облаков точек
36ac9f2 Журнал деплоя: 2a5afad
```
...
## 2026-08-26 23:31 — hybrid-v8 @ 2a5afad

**Коммиты в этой поставке**
```
2a5afad Пояса астероидов: метеоритное пространство вплетено в бесконечный полёт
5cfea1c Журнал деплоя: 5106c0d
```
...
**Коммиты в этой поставке**
```
dd19d76 Экран результата: убит iOS-автозум, свой скролл; имя закреплено за игроком
a5ecbd7 Журнал деплоя: e6eb643
```
...
## 2026-08-26 23:02 — hybrid-v8 @ e6eb643

**Коммиты в этой поставке**
```
e6eb643 Рекорды во весь экран; крестик закрытия — двойник игрового, не прыгает по углам
98df2a0 Журнал деплоя: 310871f
```
...
**Коммиты в этой поставке**
```
7fd36a8 Видео поверх игры: гасим и второй слой — «чёткий кадр» #frame
81fbca8 Журнал деплоя: 19c7678
```
...
## 2026-08-26 22:07 — hybrid-v8 @ 19c7678

**Коммиты в этой поставке**
```
19c7678 Кнопки экрана результата и крестик выхода срабатывают по touchend
c41454b Журнал деплоя: 60d7324
```
...
## 2026-08-26 21:55 — hybrid-v8 @ 60d7324

**Коммиты в этой поставке**
```
60d7324 Бегунок чувствительности руля на экране результата: заводская — 65%
e8a1fd5 Шар взят из /digital/ как есть; виджет не показывается на первом экране
e10512d Журнал деплоя: ad46346
```
...
**Коммиты в этой поставке**
```
6564a5c Руль v6: вертикаль нормирована по пропорции экрана
d439933 Знание: превью в Telegram не строятся из-за сетевой блокировки, а не разметки
46fe4b6 Журнал деплоя: 5305343
```

**На сервер отправлено файлов:** 122, удалено: 0
```
hero-scene.html
index.html
llms.txt
...
## 2026-08-26 20:26 — hybrid-v8 @ 5305343

**Коммиты в этой поставке**
```
5305343 Деплой: временно оберегаем tg-probe соседней проверки превью
f42ba4b Руль v6 — относительный тач, как в Sky Force: джойстик убран совсем
6fb3da0 Журнал деплоя: 5cd6e0d
```
...
## 2026-08-26 20:14 — hybrid-v8 @ 5cd6e0d

**Коммиты в этой поставке**
```
5cd6e0d Пульс состояния сцены: залипшие video-on/game-on больше не прячут интерфейс
26887a3 Журнал деплоя: e92e136
```
...
## 2026-08-26 19:56 — hybrid-v8 @ e92e136

**Коммиты в этой поставке**
```
e92e136 Журнал деплоя: f464efc
```
...
**Коммиты в этой поставке**
```
7bac7f8 Обвязка сайта на все разделы: услуги, концепции, атлас
dbc7d00 Журнал деплоя: 1c4ceec
```
...
## 2026-08-26 16:16 — hybrid-v8 @ 1c4ceec

**Коммиты в этой поставке**
```
1c4ceec Аналитика: свои визиты не считаем
f511786 Журнал деплоя: e49bb1f
```
...
## 2026-08-26 16:04 — hybrid-v8 @ e49bb1f

**Коммиты в этой поставке**
```
e49bb1f посадочная: техническое оснащение музея
e326a90 Девятая посадочная: техническое оснащение музея
0f0aa8c Журнал деплоя: b9bc334
```
...
## 2026-08-26 15:58 — hybrid-v8 @ b9bc334

**Коммиты в этой поставке**
```
b9bc334 Аналитика: воронка раздела оборудования
e0ff33e Журнал деплоя: 9c0278c
```
...
## 2026-08-25 11:52 — hybrid-v8 @ 9c0278c

**Коммиты в этой поставке**
```
9c0278c Якорь при заходе извне: прыжок мгновенный, иначе не доезжал
31e3f01 Журнал деплоя: c9f1038
```

**На сервер отправлено файлов:** 119, удалено: 0
```
index.html
llms.txt
robots.txt
...
## 2026-08-25 11:43 — hybrid-v8 @ c9f1038

**Коммиты в этой поставке**
```
c9f1038 Раздел: анимированная подача, Ventuz и vvvv, рабочие кнопки
334ab1e Галерея листается не чаще, чем едет сама лента — одно правило вместо двух
eb9da77 Журнал деплоя: 0771d30
```
...
## 2026-08-25 11:27 — hybrid-v8 @ 0771d30

**Коммиты в этой поставке**
```
0771d30 Бегунок не листает слайд, состояние ролика по центру, знаки по центру плиток
7827e54 Бегунок больше не листает слайд, у ролика видно загрузку и пуск, знаки по центру плиток
53a0991 Журнал деплоя: 2ddb2f4
```
...
## 2026-08-25 11:12 — hybrid-v8 @ 2ddb2f4

**Коммиты в этой поставке**
```
2ddb2f4 Раздел оборудования: шапка один в один с главной, шрифты сайта; блок «Клиенты»
837dfbb Журнал деплоя: 686b08a
```
...
## 2026-08-25 10:49 — hybrid-v8 @ 686b08a

**Коммиты в этой поставке**
```
686b08a Экономика контакта: 15–18 ₽ за час вместо одной цифры
5d2a500 Журнал деплоя: af702e9
```
...
**Коммиты в этой поставке**
```
4e8b4c9 Слайд оборудования: числа не помещались в текстовую колонку
a1b074b Журнал деплоя: 7bcc1dc
```
...
**Коммиты в этой поставке**
```
6c8871d заголовок блока о студии в библиотеке
57f3b47 Библиотека: заголовок блока о студии — «Интеграция от Playdisplay»
ca70bb7 Журнал деплоя: f83135a
```
...
**Коммиты в этой поставке**
```
003c416 Журнал: расширение библиотеки, английская версия, спрос по новым темам
05afb15 Журнал деплоя: 1692426
```
...
## 2026-08-24 18:42 — hybrid-v8 @ 1692426

**Коммиты в этой поставке**
```
1692426 Библиотека: 63 позиции, английская версия, светодиод отдельным разделом
ab31c79 Журнал деплоя: ac5dd92
```
...
## 2026-08-24 18:12 — hybrid-v8 @ ac5dd92

**Коммиты в этой поставке**
```
ac5dd92 Журнал: опечатка, английское слово в русском тексте
94f5207 Библиотека: иконки категорий, нарядные карточки, новое позиционирование
15d59c8 Журнал деплоя: 652297e
```

**На сервер отправлено файлов:** 116, удалено: 0
```
llms.txt
robots.txt
sitemap.xml
...
## 2026-08-24 16:41 — hybrid-v8 @ 00fbbd4

**Коммиты в этой поставке**
```
00fbbd4 новая посадочная: оборудование для музея (RU+EN), под запрос 602/мес
72874ce Журнал деплоя: d2a9b3b
```

**На сервер отправлено файлов:** 115, удалено: 0
```
index.html
llms.txt
robots.txt
...
**Коммиты в этой поставке**
```
d903589 Джойстик: iPad опознан, скорость руля в настройках; секретная панель и общие настройки сцены
2646e5f deploy.sh отказывается стартовать поверх чужой заливки
5d09fdb Журнал деплоя: 88e6de6
2951813 Результаты Сталинграда, форума, «Панорамы 360» и МИГ-35: 9 кейсов из 15
```

**На сервер отправлено файлов:** 111, удалено: 0
```
hero-scene.html
index.html
llms.txt
...
## 2026-08-23 21:58 — hybrid-v8 @ 88e6de6

**Коммиты в этой поставке**
```
2951813 Результаты Сталинграда, форума, «Панорамы 360» и МИГ-35: 9 кейсов из 15
88e6de6 WebP рядом с картинками: 35 -> 24 МБ на статических страницах
3327e78 Журнал деплоя: f22076e
```

**На сервер отправлено файлов:** 357, удалено: 0
```
hero-scene.html
index.html
llms.txt
...
## 2026-08-23 15:54 — hybrid-v8 @ f22076e

**Коммиты в этой поставке**
```
f22076e Три последних дубля заголовков разведены направлением
76475ad Журнал деплоя: 7f31cba
```
...
## 2026-08-23 15:40 — hybrid-v8 @ 7f31cba

**Коммиты в этой поставке**
```
7f31cba Деплой из чата работает — три документа врали об обратном
da6704d Журнал деплоя: 4a9137b
```
...
## 2026-08-23 15:29 — hybrid-v8 @ 4a9137b

**Коммиты в этой поставке**
```
4a9137b Журнал деплоя: 78a93f4
```
...
## 2026-08-23 15:28 — hybrid-v8 @ 78a93f4

**Коммиты в этой поставке**
```
78a93f4 Единственный адрес студии — a.sudarikov@playdisplay.ru, rolltorule убран
952e2a9 Журнал деплоя: ae3f0f5
```
...
## 2026-08-23 15:24 — hybrid-v8 @ ae3f0f5

**Коммиты в этой поставке**
```
ae3f0f5 Бриф ассистента — на a.sudarikov@playdisplay.ru
64dae05 Журнал деплоя: 538a602
```
...
## 2026-08-23 15:12 — hybrid-v8 @ 538a602

**Коммиты в этой поставке**
```
538a602 Журнал деплоя: 2ac2c4a
```
...
## 2026-08-23 15:07 — hybrid-v8 @ 2ac2c4a

**Коммиты в этой поставке**
```
2ac2c4a Журнал: разбор письма Google об индексировании
f39d42c МИГ-35: полный фильм в кейсе, ливрея без троекратного повтора
5c8eca6 Четыре семейства старых адресов вместо шестнадцати 404
44d0dac Стенд МИГ-35 показывал ролик ОДК: перепутана адресация, ничего не потеряно
cb1177f Журнал деплоя: 028012d
```

**На сервер отправлено файлов:** 114, удалено: 0
```
.htaccess
index.html
llms.txt
...
**Коммиты в этой поставке**
```
e4080e9 Канонический домен вернулся на .com
f60accf CLAUDE.md: раздел про посадочные страницы услуг
d7cff3b 301 с мёртвых адресов старого вордпресса вместо 404
1f32cf6 Рабочий документ по продвижению: замер, сделанное, внешние площадки
40f48ca Английские посадочные: /en/services/ и шесть строк в словарь
1ec725d Посадочные страницы услуг: /services/ и пять направлений
f8c9955 Бриф на диктовку блока «Результат» по 15 кейсам
f7ad8b2 Техстраницы убраны из поиска, описания больше не рвутся посреди слова
923a21e Журнал деплоя: 86e2ba9
```

**На сервер отправлено файлов:** 68, удалено: 0
```
.htaccess
hero-scene.html
index.html
...
en/services/
en/services/index.html
en/services/exhibition-stand/
en/services/exhibition-stand/index.html
en/services/interactive-exhibition/
en/services/interactive-exhibition/index.html
en/services/museum-concept/
en/services/museum-concept/index.html
en/services/projection-mapping/
en/services/projection-mapping/index.html
en/services/showroom/
en/services/showroom/index.html
en/work/airports/index.html
en/work/bmwx5/index.html
en/work/coalco/index.html
...
## 2026-08-20 23:55 — hybrid-v8 @ 86e2ba9

**Коммиты в этой поставке**
```
86e2ba9 Телефон: слайд по содержанию, кадры двумя строками, плитки в две колонки
8818455 Презентации отвечают на обоих доменах — теперь это записано правилом, а не случайностью
9233a31 Деплой: прогресс заливки виден в окне, а не уходит в переменную
e5f67a0 Журнал деплоя: 99327a4
```
...
## 2026-08-20 19:51 — hybrid-v8 @ 99327a4

**Коммиты в этой поставке**
```
99327a4 Деплой: видно прогресс, недокачанное не выбрасывается, мёртвая связь падает
468c10a Целевые метрики держат экран крупными цифрами; один свайп — один кадр
a667be9 Музыка с явным выключателем, прокрутка листает галереи, кубики отвечают на курсор
ed38201 На первом экране живая игра: захват геймплея вместо стопки скриншотов
6286fc0 Габаритов ролика не видно: поле слайда ровно того цвета, что подложка внутри видео
be6e334 Финал: куб — фигура — куб тремя слоями с покачиванием; край ролика больше не квадрат
40efd16 Целевые метрики заполнены, числа первого экрана перестали съезжать
636ab8d Журнал деплоя: 209fcd6
```

**На сервер отправлено файлов:** 12, удалено: 0
```
avtovaz/assets/hero-gameplay.mp4
avtovaz/assets/music-bg.mp3
avtovaz/private/page.html
digital/index.html
kit/blocks.json
kit/catalog.html
...
## 2026-08-19 17:19 — hybrid-v8 @ 209fcd6

**Коммиты в этой поставке**
```
209fcd6 Журнал деплоя: 7a54fa8
```
...
- 2026-09-07 09:44 — POEMIQ: общий набор побеждает устаревший локальный. Точечная заливка poemiq (rsync без --delete; удаление старых чанков MARÉ всё ещё ждёт подтверждения владельца). Коммит 3a95ed6.

- 2026-09-07 10:14 — POEMIQ: прелоадер. Точечная заливка poemiq (rsync без --delete). Коммит b918273.
